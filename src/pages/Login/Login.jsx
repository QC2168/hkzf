...
import {
  Flex, WingBlank, WhiteSpace, Toast,
} from 'antd-mobile';
...
const config = {
  ...
  handleSubmit: async (values, formikBag) => {
    ...
    const { status, description, body } = await API.post('user/login', { username, password });
    if (status === 200) {
      // 成功了
      // 1. 先把token存储起来
  ...
  validationSchema: Yup.object().shape({
    username: Yup.string()
      ...
      .matches(
        ...
        '用户名长度为4到8位，只能出现数字、字母、下划线',
      ),
    password: Yup.string()
      .required('密码是必填的')
      .matches(
        /^[a-zA-Z0-9]{3,12}$/,
        '密码必须是4-12位，由数字，字母，下划线组成,不能以数字开头',
      ),
  }),
